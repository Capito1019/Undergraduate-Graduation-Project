...
class LoRA_CodeLlamaChat:
    ...
    def load_lora_weights(self, lora_weights_path: str):
        """使用 PEFT 加载 LoRA 微调权重"""
        print(f"🔄 正在加载 LoRA 权重: {lora_weights_path}")
...
def main():
    # 设定 JSON 文件路径
    json_file_path = "/data/capito/a_bishe/bench/js_humanevalpack.jsonl" #bench
    output_file_path = "/data/capito/a_bishe/LoRA_infer/js_LoRA_fixed_tasks.json"  # 纯二维列表格式
    used_json_path = "/data/capito/a_bishe/LoRA_infer/js_LoRA_used_tasks.json"  # 记录已处理的 task_id
    ...
    #         print(f"❌ 发现 NaN/Inf 参数: {name}")

    # chat_model.model.to("cpu")  # 把模型转移到 CPU
            ...
            try:
                ...
                for response_set in responses:
                    for response in response_set["generation"]:
                        ...
                        extracted_code = re.findall(r"```js\n(.*?)\n```", response_text, re.DOTALL)
                        ...
                        if not extracted_code:
                            if "js\n" in response_text:
                                extracted_code = [response_text.split("js\n", 1)[1].strip()]
