    ...
    for cur_pos in range(min_tokens_len, total_len):
        ...
        for output in outputs:
            ...
            if hasattr(output, "router_logits"):
                for layer_id, layer_router_logits in enumerate(output.router_logits):
                    # shape: [B, T, E]
                    router_weights = torch.softmax(layer_router_logits, dim=-1)

                    if router_weights.dim() == 3:
                        weights_sum = router_weights.sum(dim=(0, 1))
                    elif router_weights.dim() == 2:
                        weights_sum = router_weights.sum(dim=0)
                    elif router_weights.dim() == 1:
                        weights_sum = router_weights
                    else:
                        raise ValueError(f"Unexpected router_weights shape: {router_weights.shape}")

                    # 自动识别名称（如 q_proj/k_proj），否则 fallback 到 layer_id
                    if hasattr(output, "router_proj_names"):
                        proj_name = output.router_proj_names[layer_id]
                    else:
                        proj_name = f"layer_{layer_id}"

                    if proj_name not in expert_stats:
                        expert_stats[proj_name] = weights_sum.detach().cpu()
                    else:
                        expert_stats[proj_name] += weights_sum.detach().cpu()
...
def generate(
    ...
    # 👇 Soft routing 权重累积容器
    expert_stats = {}  # key: proj_name ("q_proj", etc), value: Tensor[num_experts]
    ...
    while True:
        ...
        outputs, running_jobs = _batch_generate(
            ...
            config_dict,
            *dispatch_args,
            expert_stats=expert_stats  # 👈 加上传统收集容器
        )
    ...
    # 👇 Soft routing 统计归一化输出
    normalized_stats = {
        proj: {
            f"expert_{i}": round(v.item(), 6)
            for i, v in enumerate(tensor / tensor.sum())
        }
        for proj, tensor in expert_stats.items()
    }

    return packed_outputs, normalized_stats